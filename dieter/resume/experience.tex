...
%---------------------------------------------------------
\cventrywithsummary
{Lead ML Ops Engineer} % Job title
{Apptronik} % Organization
{Remote} % Location
{Feb 2026 - Present} % Date(s)
{
  \begin{cvitems} % Description(s) of tasks/responsibilities
    \item {Planned and built on-premises AI factory infrastructure (Kubernetes, MinIO, LakeFS, PostgreSQL) supporting 10 researchers and engineers across humanoid robotics training workflows.}
    \item {Successfully deployed \$2M of on-premises GPU hardware and led a seamless migration with no disruption to research workflows.}
    \item {Designed and operated the AI factory compute cluster spanning on-premises GPU hardware and GCS, with full observability via VictoriaMetrics, VictoriaLogs and Grafana.}
    \item {Building and leading the ML Ops function end-to-end, including hiring to grow the team.}
    \item {Developed custom tooling for data annotation (CVAT), ML experiment tracking (WandB), compute scheduling, and robotics visualization (Rerun) for IsaacLab training workflows.}
  \end{cvitems}
}
{
  Apptronik is a humanoid robotics company building Apollo, a general-purpose robot designed to work alongside humans in industrial and commercial environments. Backed by Google and B Capital, Apptronik has raised over \$935M at a \$5B+ valuation.
}
...
{
  \begin{cvitems} % Description(s) of tasks/responsibilities
    \item {Led a team of 5 DevOps \& MLOps engineers supporting Sanctuary AI's cognitive architecture and Phoenix humanoid robot platform.}
    \item {Reduced cognitive architecture build size by 10x, enabling rapid iteration and deployment cycles on the Phoenix humanoid platform.}
    \item {Built an on-demand simulation asset delivery service to accelerate research and development iteration.}
    \item {Designed and automated robot-to-cloud data offload for the Phoenix fleet, eliminating operational toil without disrupting robot operations.}
...
{
  \begin{cvitems} % Description(s) of tasks/responsibilities
    \item {Built DevOps pipelines and microservice infrastructure to deploy software across IoT edge devices and cloud services for a bespoke ML-enhanced hydroponic farming system.}
  \end{cvitems}
}
{
...
{
  \begin{cvitems} % Description(s) of tasks/responsibilities
    \item {For HSBC International I evolved their mobile banking build infrastructure \& release management for an application serving tens of millions of users, with 1,000+ builds/day on a 200+ node hybrid Jenkins cluster producing 43 localized variants on iOS and Android. Built custom observability and release management tooling and hired \& mentored the Canadian engineering team.}
    \item {For Tally Inc. I migrated their primary website from an unmaintained WordPress installation to a Contentful-based solution on a highly available ECS cluster, with a CI/CD pipeline for safe, frequent updates. Also mentored engineers on cloud infrastructure best practices including consolidating 5 VPNs into a single role-based system.}
  \end{cvitems}
}
{
...
{
  \begin{cvitems} % Description(s) of tasks/responsibilities
    \item {Took over the legacy data platform from the San Francisco office; achieved a 4x reduction in storage cost and 2x increase in resampling performance across back-end services and data packaging.}
    \item {Co-developed an automated pipeline to migrate more than a petabyte of indexed geospatial data from raw S3 to a compressed, Glacier-archived format.}
    \item {Co-authored a successful \$1.1M grant proposal for Canada's Innovation Supercluster initiative — a public-private-academic partnership with UBC and Microsoft Canada.}
    \item {Key contributor to architecting the next-generation optical processing pipeline, data repository, calibration and validation system.}
  \end{cvitems}
}
{
